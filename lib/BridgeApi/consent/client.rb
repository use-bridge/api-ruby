...
module BridgeApi
  module Consent
    class Client
      ...
      #
      # @return [BridgeApi::Consent::Types::PatientConsentsListV1Response]
      def list_patient_consents(request_options: {}, **params)
        ...
        if code.between?(200, 299)
          (response.body.to_s.empty? ? nil : BridgeApi::Consent::Types::PatientConsentsListV1Response.load(response.body))
        else
      ...
      # @param request_options [Hash]
      # @param params [BridgeApi::Consent::Types::PatientConsentCreateV1Request]
      # @option request_options [String] :base_url
      # @option request_options [Hash{String => Object}] :additional_headers
      # @option request_options [Hash{String => Object}] :additional_query_parameters
      # @option request_options [Hash{String => Object}] :additional_body_parameters
      ...
      #   )
      #
      # @return [BridgeApi::Consent::Types::PatientConsentCreateV1Response]
      def create_patient_consent(request_options: {}, **params)
        ...
        request = BridgeApi::Internal::JSON::Request.new(
          ...
          method: "POST",
          path: "/api/patients/#{URI.encode_uri_component(params[:id].to_s)}/consent",
          body: BridgeApi::Consent::Types::PatientConsentCreateV1Request.new(body_params).to_h,
          request_options: request_options
        )
        begin
          response = @client.send(request)
        ...
        code = response.code.to_i
        if code.between?(200, 299)
          (response.body.to_s.empty? ? nil : BridgeApi::Consent::Types::PatientConsentCreateV1Response.load(response.body))
        else
          error_class = BridgeApi::Errors::ResponseError.subclass_for_code(code)
          raise error_class.new(response.body, code: code)
        end
